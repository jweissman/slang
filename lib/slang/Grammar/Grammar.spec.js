...
describe('Grammar', () => {
  ...
  it('hash', () => {
    ...
    expect(grammar.match('hi.there=3').succeeded()).toBe(true)
  })
  ...
  it('funcalls without parens', () => {
    expect(grammar.match('foo 1, b, 2+3').succeeded()).toBe(true)
  })

  it('comparison operator', () => {
    expect(grammar.match('a>b').succeeded()).toBe(true)
    expect(grammar.match('1+2*x>y*4').succeeded()).toBe(true)
  })

  it('ternary conditions', () => {
    expect(grammar.match('a ? b : c').succeeded()).toBe(true)
  })

  it('cardinality literal', () => {
    expect(grammar.match("#[1,2,3] + #'hello'").succeeded()).toBe(true)
  })

  xit('argless defun', () => {
    expect(grammar.match('fn = -> p 123').succeeded()).toBe(true)
  })
})
