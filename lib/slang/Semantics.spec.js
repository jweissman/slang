  ...
  describe('parses, yielding an ast', () => {
    describe('binary expressions', () => {
      ...
      it('negates', () => {
        let expected = p(
          binaryExpr(
            '+',
            negated(parens(binaryExpr('*', intLit('4'), intLit('2')))),
            negated(intLit('4'))
          )
        )
    ...
    describe('parentheses', () => {
      it('parses parenthetical expressions', () => {
        let expected = p(parens(binaryExpr('+', intLit('4'), intLit('3'))))
      ...
      it('parses function applications', () => {
        ...
        expected = p(
          funCall(
            ...
            argList([
              intLit('2'),
              binaryExpr('+', intLit('1'), funCall(idExpr('bar'))),
            ])
          )
        )
        expect(tree('foo(2,1+bar())')).toEqual(expected)
    ...
    describe('values (assignment)', () => {
      it('parses assignments', () => {
        let expected = p(
          assignment(idExpr('a'), binaryExpr('+', idExpr('b'), idExpr('c')))
        )
    ...
    describe('hash notation', () => {
      it('parses a hash literal', () => {
        let expected = p(hshLit([kvPair(idExpr('hello'), strLit('world'))]))
        expect(tree("{hello:'world'}")).toEqual(expected)
      })
      it('parses dot access notation', () => {
        let expected = p(dotAccess(idExpr('item'), idExpr('attr')))
        expect(tree('item.attr')).toEqual(expected)
      })

      it('parses nested dot notation', () => {
        let expected = p(
          dotAccess(dotAccess(idExpr('obj'), idExpr('item')), idExpr('attr'))
        )
    ...
    describe('multiline fn', () => {
      it('parses block notation', () => {
        let expected = p(defun(argList([]), block(p(intLit('1'), intLit('2')))))
        expect(tree('()=>{1; 2}')).toEqual(expected)
      })
    })
    ...
    describe('turnstiles', () => {
      it('parses turnstiles', () => {
        // right tack
        let expected = p(turnstile('yield', idExpr('value')))
        expect(tree('|- value')).toEqual(expected)

        // left tack
        expected = p(turnstile('derive', idExpr('gen')))
        expect(tree('-| gen')).toEqual(expected)

        // bind
        expected = p(turnstile('stream', funCall(idExpr('f'), argList([]))))
        expect(tree('&f()')).toEqual(expected)

        // star tack
        expected = p(turnstile('bubble', idExpr('gen')))
        expect(tree('|* gen')).toEqual(expected)
      })
    })
  })
