    ...
    evaluate(vm) {
        ...
        if (vm.hasBuiltin(fnName)) {
            ...
            if (ref.ctx) {
                ...
                // ref does not seem to be a function, but? just hand it back for lambda's sake?
                // really we are [want to be] sending the vm a 'message' here maybe, to the global object in this ctx 'frame'
                return ref;
            }
        } else {
            // result = vm.lookupVal
...
class LambdaFuncall extends AstNode {
    constructor(meth, args) { super({ meth, args })}
    evaluate(_vm) {
        throw new Error("lambda funcall eval not impl")
    }
}
...
const helpers = {
  ...
  lambdaFuncall: (fn, args = new ArgList([])) => new LambdaFuncall(fn, args),
}
