...
const std = function(vm) {
  return {
    ...
    p: elems => {
      console.log(...elems.map(e => !!e ? e.pretty() : 'undefined?'))
      return new Null()
    },
    rest: elems => {
      return elems[0].rest()
...
class VM {
  ...
  storeValue(key, value) {
    // console.log("Store", value, "in key", key)
  ...
  callAnonymousFunction(fn, params, slangEval) {
    if (fn.kind && fn.kind === 'builtin') {
      return this.builtin(fn.name).call(null, params)
    } else {
      ...
      // console.log("VM -- about to eval expr", { expr })
      // debugger;
