...
function* slangEval(node) {
  ...
  if (node instanceof Program) {
    ...
    console.log("TURNSTILE", { tack, e })
    if (tack === 'yield') {
      let val = syncSlangEval(e)
      console.log("yield", val)
      yield val
      result = new Null() //val // do we have to set result to smth non-Null?
    } else if (tack == 'stream') {
      ...
      result = yield *slangEval(e)
      console.log("bubbled", { result })
    }
  } 
    ...
    let key = k.ref
    let val = syncSlangEval(v) //v.evaluate(vm)
    result = [key, val]
  }
